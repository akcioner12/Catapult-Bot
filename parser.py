"""
Автопарсер контента v8 (РАЗДЕЛЁННАЯ ВЕРСИЯ — только парсер/одобрение)
...
ВАЖНО: этот файл слушает PARSER_BOT_TOKEN (@Parser_catapult_bot).
Весь функционал для обычных пользователей (warmup, квиз, /start, /connect)
живёт в отдельном файле bot.py и слушает MAIN_BOT_TOKEN (@catapulttrade_guide_bot).
Логика парсинга/одобрения/публикации НЕ менялась — перенесена как есть.
"""
...
MAIN_BOT_TOKEN   = os.getenv("BOT_TOKEN")  # используется только для публикации в канал
...
async def handle_edit_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """В этом боте сообщения пишет только админ — обычных пользователей здесь нет."""
    user_id = update.effective_user.id
    if user_id != ADMIN_TG_ID:
        return
...
async def global_error_handler(update, context: ContextTypes.DEFAULT_TYPE):
    """Логирует любые необработанные исключения внутри handler'ов."""
...
async def debug_log_all_updates(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"🔔 RAW UPDATE RECEIVED: user_id={uid} kind={kind}")
    except Exception as e:
        logger.info(f"🔔 RAW UPDATE RECEIVED but failed to parse: {e}")


async def main():
    if not PARSER_BOT_TOKEN:
        raise RuntimeError("PARSER_BOT_TOKEN не задан в переменных окружения!")

    app = Application.builder().token(PARSER_BOT_TOKEN).build()

    app.add_error_handler(global_error_handler)

    # Диагностика: логируем каждое обновление первым
    ...
    scheduler.start()
    logger.info("✅ Parser (отдельный процесс, PARSER_BOT_TOKEN) запущен!")
